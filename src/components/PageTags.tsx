...
export function PageTags({ content, onChange }: Props) {
  const tags = useMemo(() => getNoteTags(content), [content]);
  ...
  const commitTag = (name: string) => {
    ...
    const next = setNoteTags(content, [...tags, nextName]);
    onChange(next);
    setDraft("");
  };
  ...
  return (
    <div className="page-tags" ref={rootRef}>
      <div className="page-tags-chips" role="list" aria-label="Page tags">
        {tags.map((tag) => (
          ...
          {open ? (
            <div className="page-tags-popover" role="dialog" aria-label="Add tag">
              <input
                ...
                aria-controls="page-tags-suggestions"
                onChange={(e) => {
