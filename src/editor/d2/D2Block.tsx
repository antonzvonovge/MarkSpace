...
import { renderD2ToSvg } from "./renderD2";

export const DEFAULT_D2_CODE = `direction: down
start: Start
end: End
start -> end`;
...
function D2Preview({ code, dark }: { code: string; dark: boolean }) {
  ...
  return (
    ...
    >
      <div className="diagram-block__toolbar">
        <span className="diagram-block__label">D2</span>
      ...
      {mode === "edit" ? (
        <textarea
          ...
          placeholder={DEFAULT_D2_CODE}
          rows={Math.min(16, Math.max(4, draft.split("\n").length + 1))}
        />
      ) : (
        <D2Preview code={block.props.code} dark={dark} />
      )}
      <DiagramLightbox
        open={lightboxOpen}
        engine="d2"
