...
Conflicts appear in Settings → Sync and as a banner in the editor. Markdown (`.md`) conflicts are resolved automatically with semantic frontmatter merge (tags union, earliest `created`, latest `updated`, one heading, both sides' content). Draw.io (`.drawio`) diagrams are never auto-merged — choose **Keep mine** or **Keep theirs**. Delete/delete conflicts are accepted automatically; modify/delete keeps the edited version. For other remaining conflicts choose **Keep both**, **Keep mine** / **Keep theirs**, or open the file and edit conflict markers manually, then sync again.
