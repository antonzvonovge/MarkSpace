...
4. If resumption depends on the checkpoint, pass a `--resume` flag so the session can restart from disk
...
3. Persist session state to disk with a unique identifier (e.g., `--resume`)
