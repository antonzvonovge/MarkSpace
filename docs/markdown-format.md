...
Top-level **projects** with project type **Media library** are a personal film/series catalog. Opening any folder under such a project shows a recursive **tile catalog** (search, kind/genre filters). Context menu **New film…** looks up **Kinopoisk** when the profile native language is Russian (or the query is Cyrillic), otherwise **OMDb** for Latin titles. API keys live in Settings → Media library. In Agent mode, `run_specialist` kind=`media` uses the same lookup (`search_movies` / `get_movie_details` / `create_film_note`), plus `list_media_catalog` to browse/filter cards and `ensure_folder` / `move_path` to reorganize by genre (do not hand-rewrite note-relative `.assets/` poster links after a move). Each title is one `.md` note filed into a **genre shelf** under the project root (reuse an existing child folder whose name matches any genre, case-insensitive; otherwise create `{FirstGenre}`; no genres → project root). Auto file name `{year}-{title}` (localized title, else original).
...
Short personal note about the title.
```

- `title`: localized / native-language name.
- `original_title`: title in the original language when known.
